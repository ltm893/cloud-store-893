...
const { CASH_MODE } = require('./cash-till-config');

function registerSupervisorRoutes(app, {
  loginApprovalStore,
  shiftCloseStore,
  tillStore,
  posSessionStore,
  ...
  if (tillStore && shiftCloseStore) {
    app.get('/api/admin/open-tills', requireSupervisor, asyncHandler(async (req, res) => {
      const tills = await tillStore.listOpenTills();
      const items = await Promise.all(
        tills.map(async (till) => {
          const pendingClose = await shiftCloseStore.findPendingForTill(till.id);
          let expectedCloseFloat = null;
          let cashSalesTotal = null;
          let changeGivenTotal = null;
          if (till.cashMode !== CASH_MODE.CREDIT_ONLY) {
            const totals = await shiftCloseStore.shiftCloseCash.computeExpectedClose(till);
            expectedCloseFloat = totals.expectedClose;
            cashSalesTotal = totals.cashSalesTotal;
            changeGivenTotal = totals.changeGivenTotal;
          }
          return {
            ...till,
            pendingClose: pendingClose
              ? {
                  closeToken: pendingClose.closeToken,
                  status: pendingClose.status,
                  countedCloseFloat: pendingClose.countedCloseFloat,
                  closeVariance: pendingClose.closeVariance,
                }
              : null,
            expectedCloseFloat,
            cashSalesTotal,
      ...
      items.sort((a, b) => String(a.openedAt || '').localeCompare(String(b.openedAt || '')));
      return res.json({ items });
    }));

    app.post('/api/admin/open-tills/:tillId/force-close', requireSupervisor, asyncHandler(async (req, res) => {
      const tillId = Number(req.params.tillId);
      if (!Number.isFinite(tillId) || tillId <= 0) {
        return res.status(400).json({ error: 'tillId is required' });
      }
      const reason = req.body?.reason;
      const result = await shiftCloseStore.forceClose(tillId, req.supervisorClaims, { reason });
      if (result.till?.posSessionId && posSessionStore) {
        await posSessionStore.end(result.till.posSessionId);
      }
      return res.json({
        ok: true,
        tillId: result.till?.id ?? tillId,
        registerId: result.till?.registerId ?? null,
        cashierEmail: result.till?.cashierEmail ?? null,
        closeToken: result.closeToken,
        expectedCloseFloat: result.expectedCloseFloat,
        cashSalesTotal: result.cashSalesTotal,
        creditSalesTotal: result.creditSalesTotal,
        closedBy: req.supervisorClaims?.email ?? null,
      });
    }));
  }
