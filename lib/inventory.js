...
async function lookupProductByQuery(ordsGet, rawQuery) {
  ...
  if (!product) {
    const filter = encodeURIComponent(JSON.stringify({ barcode: { $eq: query } }));
    const byBarcode = await ordsGet(`products/?q=${filter}`);
    product = byBarcode.length > 0 ? byBarcode[0] : null;
  }
  if (!product) {
