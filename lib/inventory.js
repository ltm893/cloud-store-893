...
const { barcodeLookupCandidates } = require('./barcode-normalize');

/** Retail shelf types that use product_inventory (excludes made coffee → bulk). */
const RETAIL_TRACKED_PRODUCT_TYPES = new Set([
  'coffee beans',
  'go cups',
  'clothes',
  'water',
...
function tracksInventory(productRow) {
  if (Number(productRow?.track_inventory) === 1) return true;
  const type = String(productRow?.product_type ?? '').trim();
  return RETAIL_TRACKED_PRODUCT_TYPES.has(type);
}

async function fetchProductById(ordsGet, productId) {
  const pid = Number(productId);
  if (!Number.isFinite(pid) || pid < 1) return null;
  const filter = encodeURIComponent(JSON.stringify({ id: { $eq: pid } }));
  const rows = await ordsGet(`products/?q=${filter}`);
...
function canFulfillQuantity(productRow, inventoryMap, requestedQty) {
  ...
  if (qty > available) {
    ...
    const maxOrderable = Math.max(0, Math.floor(available));
    return {
      ok: false,
      error: available > 0
...
/** Oracle/ORDS may return tax_exempt as 0/1, bool, or string. */
function isProductTaxExempt(productRow) {
  const value = productRow?.tax_exempt;
...
/** cart_view rows may omit tax_exempt when ORDS metadata is stale — fall back to products. */
function resolveCartLineTaxExempt(cartRow, productRow) {
  const fromCart = cartRow?.tax_exempt;
  if (fromCart !== undefined && fromCart !== null && fromCart !== '') {
    return isProductTaxExempt(cartRow);
  }
  if (productRow) return isProductTaxExempt(productRow);
  return false;
}
...
function mapProductForCashier(productRow, inventoryMap) {
  ...
  return {
    ...
    onSale,
    taxExempt: isProductTaxExempt(productRow),
...
function mapProductForInventoryLookup(productRow, inventoryMap) {
  const base = mapProductForCashier(productRow, inventoryMap);
  const tracked = tracksInventory(productRow);
  const inv = inventoryForProduct(inventoryMap, productRow.id);
  const reorderPoint = inv ? Number(inv.reorder_point) || 0 : null;
  const quantityOnHand = base.quantityOnHand;
  return {
    ...base,
    trackInventory: tracked,
    productType: productRow.product_type ?? null,
    manufacturer: productRow.manufacturer ?? null,
    reorderPoint,
    lowStock: tracked && inv ? quantityOnHand <= reorderPoint : false,
  };
}

async function findProductByBarcode(ordsGet, rawBarcode) {
  for (const code of barcodeLookupCandidates(rawBarcode)) {
    const filter = encodeURIComponent(JSON.stringify({ barcode: { $eq: code } }));
    const rows = await ordsGet(`products/?q=${filter}`);
...
async function lookupProductByQuery(ordsGet, rawQuery) {
  ...
  if (/^\d+$/.test(query) && query.length <= 8) {
    const filter = encodeURIComponent(JSON.stringify({ id: { $eq: Number(query) } }));
    const byId = await ordsGet(`products/?q=${filter}`);
    product = byId.length > 0 ? byId[0] : null;
  }
  if (!product) {
    product = await findProductByBarcode(ordsGet, query);
  }
  if (!product) {
    return { status: 404, body: { error: 'Product not found', query } };
  }

  const inventoryMap = await loadInventoryMap(ordsGet);
  return { status: 200, body: mapProductForInventoryLookup(product, inventoryMap) };
}
