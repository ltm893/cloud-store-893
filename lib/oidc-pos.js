  ...
  registerOidcBrowserFlow(app, {
    ...
    onAuthenticated: async (req, res, { claims, flow }) => {
      ...
      const registerId = normalizeRegisterId(
        flow?.registerId || req.query?.register_id || null,
      );
      const clientKind = normalizePosClientKind(
        flow?.clientKind || req.query?.client_kind,
      ) || 'web';

      if (isListerClient(clientKind)) {
        try {
          assertValidNativeRegisterContext({ clientKind, registerId });
        } catch (err) {
          console.error(err.message);
          const status = Number(err.status) || 400;
          return res.status(status).send(err.message || 'Invalid lister register');
        }
        ...
        return res.redirect(302, '/?lister_signed_in=1');
      }
