...
function createTillStore(helpers) {
  ...
  async function findResumableActiveTill(registerId, { sub, email } = {}) {
    ...
    if (trimmedRegister) {
      const onRegister = await findActiveTillForRegister(trimmedRegister);
      if (onRegister && cashierMatchesTill(onRegister, identity)) {
        return onRegister;
      }
    }
    ...
    const openForCashier = (Array.isArray(rows) ? rows : [])
      .map(mapTillRow)
    ...
    if (trimmedRegister) {
      const onReg = openForCashier.filter((t) => t.registerId === trimmedRegister);
      if (onReg.length === 1) return onReg[0];
      const orphans = openForCashier.filter((t) => !t.registerId);
      if (orphans.length === 1) return orphans[0];
      return null;
    }

    return openForCashier.length === 1 ? openForCashier[0] : null;
  }
