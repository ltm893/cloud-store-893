...
function createTillStore(helpers) {
  ...
  async function listOpenTills() {
    const activeRows = await ordsGet(`${ORDS_PATH}/?q=${encodeURIComponent(JSON.stringify({
      status: { $eq: TILL_STATUS.ACTIVE },
    }))}`);
    const inProgressRows = await ordsGet(`${ORDS_PATH}/?q=${encodeURIComponent(JSON.stringify({
      status: { $eq: TILL_STATUS.IN_PROGRESS },
    }))}`);
    const merged = [
      ...(Array.isArray(activeRows) ? activeRows : []),
      ...(Array.isArray(inProgressRows) ? inProgressRows : []),
    ];
    return merged.map(mapTillRow).filter(Boolean);
  }
