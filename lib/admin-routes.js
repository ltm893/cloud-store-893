...
const { createShiftSalesStats } = require('./shift-sales-stats');
const { createStoreReports } = require('./store-reports');
const {
  tracksInventory,
  fetchProductById,
  getBulkInventoryRow,
...
function registerAdminRoutes(app, helpers) {
  ...
  const shiftSalesStats = createShiftSalesStats({ ordsGet });
  const storeReports = createStoreReports({ ordsGet });

  app.get('/api/admin/reports/sales', asyncHandler(async (req, res) => {
    const report = await storeReports.buildSalesReport({
      period: req.query.period,
      anchor: req.query.anchor ?? req.query.date ?? null,
    });
    res.json(report);
  }));

  app.get('/api/admin/reports/inventory', asyncHandler(async (req, res) => {
    const report = await storeReports.buildInventoryReport({
      period: req.query.period,
      anchor: req.query.anchor ?? req.query.date ?? null,
    });
    res.json(report);
  ...
  app.get('/api/admin/reports/OrderDetailsByOrdernumber', asyncHandler(async (req, res) => {
    try {
      const report = await storeReports.buildOrderDetailsByOrderNumber({
  ...
  app.get('/api/admin/reports/barcodes', asyncHandler(async (_req, res) => {
    const report = await storeReports.buildBarcodeReport();
    res.json(report);
  }));
  ...
  app.post('/api/admin/inventory/receive', asyncHandler(async (req, res) => {
    ...
    const product = await fetchProductById(ordsGet, productId);
  ...
  app.get('/api/admin/:table', asyncHandler(async (req, res) => {
    ...
    let normalized = normalizeAdminRows(rows, cfg);
    if (req.params.table === 'tills') {
      normalized = await shiftSalesStats.attachStatsToTills(normalized);
    }
    res.json(normalized);
  }));
  ...
  app.get('/api/admin/:table/:id', asyncHandler(async (req, res) => {
    ...
    let normalized = normalizeAdminRow(row, cfg);
    if (req.params.table === 'tills') {
      const stats = await shiftSalesStats.statsForTill(normalized?.id);
