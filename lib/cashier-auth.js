...
function attachTillToSession(session, till) {
...
async function ensureSessionTillLinked(session, tillStore, posSessionStore, registerHint = null) {
  ...
  if (till) attachTillToSession(session, till);
}
...
async function resolveLinkedActiveTill(
  ...
  attachTillToSession(session, fallback);
  return fallback;
}
...
async function sessionStatusPayload(req, res, loginApprovalStore, sessionDeps = {}) {
  ...
  if (isValidSession(sessionId)) {
    ...
    await ensureSessionTillLinked(session, tillStore, posSessionStore, registerHint);
