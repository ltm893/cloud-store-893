...
async function resumeActiveTillSession(
  res,
  till,
  { registerId = null, claims = null, pinAuth = false, posSessionId: draftPosSessionId = null },
  posSessionStore,
) {
  let posSessionId = draftPosSessionId;
  if (!posSessionId) {
    posSessionId = await createPosSessionForClaims(posSessionStore, {
      registerId,
      claims,
      pinAuth,
    });
  }
  if (posSessionStore && !posSessionId) {
    const err = new Error('Failed to start POS session');
    err.status = 500;
    throw err;
  }
  await openCashierSessionOnTill(res, till, claims, posSessionId, pinAuth);
  const cashMode = till.cashMode || till.tillType || null;
  return {
    ok: true,
    resumed: true,
    tillId: till.id,
...
async function findResumableTillForIdentity(tillStore, registerId, { pinAuth, claims } = {}) {
  if (!tillStore) return null;
  const identity = pinAuth
    ? { sub: 'pin:cashier' }
    : { sub: claims?.sub, email: claimsEmail(claims) };
  const active = await tillStore.findResumableActiveTill(registerId, identity);
  if (!active || active.status !== 'active') return null;
  if (!active.registerId && registerId) {
    return tillStore.backfillRegisterId(active.id, registerId);
  }
  return active;
}
...
async function lookupRegisterIdForSession(session, posSessionStore) {
  if (!session?.posSessionId || !posSessionStore) return null;
  const pos = await posSessionStore.getById(session.posSessionId);
  return pos?.registerId ? String(pos.registerId).trim() : null;
}

async function findOpenTillForSession(
  session,
  tillStore,
  posSessionStore,
  registerHint = null,
) {
  if (!tillStore || !session?.sub) return null;

  const identity = { sub: session.sub, email: session.email ?? null };
  let registerId = registerHint ? String(registerHint).trim() : null;
  if (!registerId) {
    registerId = await lookupRegisterIdForSession(session, posSessionStore);
  }

  const isOpenStatus = (status) => status === 'active' || status === 'in_progress';

  if (registerId) {
    const onRegister = await tillStore.findActiveTillForRegister(registerId);
    if (onRegister && isOpenStatus(onRegister.status) && cashierMatchesTill(onRegister, identity)) {
      return onRegister;
    }
    const resumed = await tillStore.findResumableActiveTill(registerId, identity);
    if (resumed) return resumed;
  }

  const openForCashier = await tillStore.findActiveTillsForCashier(identity);
  const open = openForCashier.filter((till) => till && isOpenStatus(till.status));
  if (!open.length) return null;
  if (registerId) {
    const onRegister = open.find((till) => till.registerId === registerId);
...
async function attachTillToSession(session, till, sessionId = null) {
  if (!session || !till?.id) return;
  session.tillId = till.id;
  const cashMode = till.cashMode || till.tillType || null;
  if (cashMode && !session.cashMode) {
    Object.assign(
      session,
      tillFieldsFromApproval({
        cashMode,
...
async function ensureSessionTillLinked(session, tillStore, posSessionStore, registerHint = null, sessionId = null) {
  if (!session || session.tillId || !tillStore) return;
  const till = await findOpenTillForSession(session, tillStore, posSessionStore, registerHint);
  if (till) await attachTillToSession(session, till, sessionId);
}

async function resolveLinkedActiveTill(
  session,
  tillStore,
  posSessionStore,
  registerHint = null,
  ...
  if (tillId) {
    ...
    if (till && till.status !== 'closed') return till;
  }

  const fallback = await findOpenTillForSession(session, tillStore, posSessionStore, registerHint);
  if (!fallback) {
    const err = new Error('No active till');
    err.status = 400;
    throw err;
  }
  await attachTillToSession(session, fallback);
  return fallback;
}
...
async function issueCashierSessionAfterTillCount(
  ...
  const tillFields = tillFieldsFromApproval(approvalLike);
  let tillId = await maybeCreateTill(tillStore, posSessionId, approvalLike);
  if (!tillId && tillStore && approvalLike.registerId) {
    const existing = await tillStore.findActiveTillForRegister(approvalLike.registerId);
    if (
      existing?.id
      && cashierMatchesTill(existing, {
        sub: approvalLike.cashierSub,
        email: approvalLike.cashierEmail,
      })
    ) {
      tillId = existing.id;
    }
...
async function getActiveCashierSession(req) {
  const sessionId = getSessionId(req);
  if (!(await isValidSession(sessionId))) return null;
  const session = await getSession(sessionId);
  if (session) {
    req._cashierSession = session;
    req._cashierSessionId = sessionId;
  }
  return session;
}
...
async function resolvePendingApproval(req, loginApprovalStore) {
  ...
  if (await isValidSession(getSessionId(req))) {
    return { alreadySignedIn: true, session: await getSession(getSessionId(req)) };
  }
...
function isPublicPosRead(req) {
  ...
  const path = req.path || '';
  if (path === '/api/products' || path === '/api/products/') return true;
  if (path === '/api/inventory/lookup' || path === '/api/inventory/lookup/') return true;
  if (path === '/api/systems' || path === '/api/systems/') return true;
  return false;
}

async function hasCashierAccess(req) {
  if (await isValidSession(getSessionId(req))) return true;
  const cfg = getPosConfig();
  if (!cfg) return false;
...
async function sessionStatusPayload(req, res, loginApprovalStore, sessionDeps = {}) {
  const { tillStore = null, posSessionStore = null } = sessionDeps;
  const registerHint = normalizeRegisterId(
    req?.query?.register_id || req?.query?.registerId || null,
  );
  ...
  if (await isValidSession(sessionId)) {
    const session = await getSession(sessionId);
    await ensureSessionTillLinked(session, tillStore, posSessionStore, registerHint, sessionId);
    ...
    if (sessionDeps.tillSaleGuard) {
      Object.assign(payload, await sessionDeps.tillSaleGuard.sessionFlags(session));
    }
    return payload;
  }
...
function registerCashierAuth(app, {
  ...
  app.post('/api/cashier/unlock', async (req, res) => {
    try {
      ...
      if (isCashTillConfigured()) {
        ...
        const pinClaims = { sub: 'pin:cashier' };
        if (tillStore && registerId) {
          const active = await findResumableTillForIdentity(tillStore, registerId, { pinAuth: true });
          if (active) {
            const payload = await resumeActiveTillSession(
              res,
              active,
              { registerId, pinAuth: true },
              posSessionStore,
            );
            return res.json(payload);
          }
          await guardRegisterForSignIn(tillStore, registerId, pinClaims);
        }
        ...
        const posSessionId = await createPosSessionForClaims(posSessionStore, {
          registerId,
          pinAuth: true,
        });
        if (posSessionStore && !posSessionId) {
  ...
  app.post('/api/cashier/approval/till', async (req, res) => {
    try {
      ...
      if (tillStore) {
        const resumed = await findResumableTillForIdentity(tillStore, draft.registerId, {
  ...
  async function resolveActiveTill(session, registerHint = null) {
    return resolveLinkedActiveTill(session, tillStore, posSessionStore, registerHint);
  }
  ...
  app.get('/api/cashier/shift/close/preview', async (req, res) => {
    try {
      const session = await getActiveCashierSession(req);
      if (!session) return res.status(401).json({ error: 'Cashier sign-in required' });
      const registerHint = normalizeRegisterId(
        req.query?.register_id || req.query?.registerId || null,
      );
      const till = await resolveActiveTill(session, registerHint);
      if (!shiftCloseStore) {
  ...
  app.post('/api/cashier/shift/close/till', async (req, res) => {
    try {
      ...
      const till = await resolveActiveTill(
        session,
        normalizeRegisterId(req.body?.registerId || req.query?.register_id || null),
      );
      if (ordsGet) {
        const cartItems = await ordsGet('cart_items/');
      ...
      if (!isSupervisorApprovalEnabled()) {
        try {
          ...
          if (approveErr.code !== 'NOT_PENDING') throw approveErr;
          const latest = await shiftCloseStore.findLatestForTill(till.id);
          if (!latest || latest.status !== shiftCloseStore.STATUS.APPROVED) {
            throw approveErr;
          }
        }
...
module.exports = {
  ...
  attachTillToSession,
  ensureSessionTillLinked,
  resolveLinkedActiveTill,
};
