...
function createTillCloseStore(deps) {
  ...
  async function cancelPendingWithoutReopen(tillId) {
  ...
  async function forceClose(tillId, supervisorClaims, { reason } = {}) {
    const till = await tillStore.getTillById(tillId);
    if (!till) throw makeError('Till not found', 'NOT_FOUND', 404);
    if (till.status !== TILL_STATUS.ACTIVE && till.status !== TILL_STATUS.IN_PROGRESS) {
      throw makeError(`Till is ${till.status}`, 'NOT_OPEN', 409);
    }

    await cancelPendingWithoutReopen(tillId);
    ...
    const forceReason = reason ? String(reason).trim().slice(0, 500) : null;
    ...
    await ordsPost(`${ORDS_PATH}/`, {
      ...
      register_id: till.registerId ?? null,
      cashier_sub: till.cashierSub,
      cashier_email: till.cashierEmail ?? null,
      cashier_name: null,
      till_type: cashMode,
      expected_close_float: cashTotals.expectedClose,
      counted_close_float: null,
      close_variance: null,
      close_denominations: null,
      cash_sales_total: cashTotals.cashSalesTotal,
      change_given_total: cashTotals.changeGivenTotal,
      opening_counted_float: cashTotals.openingCountedFloat,
      status: STATUS.FORCE_CLOSED,
      requested_at: resolvedAt,
      expires_at: resolvedAt,
      resolved_at: resolvedAt,
      resolved_by_sub: supervisorSub || null,
      resolved_by_email: supervisorEmail,
      deny_reason: forceReason,
    });

    const stats = await shiftCloseCash.summarizeTillSales(till.id);
    const closedTill = await tillStore.closeTill(till.id, {
      cashSales: stats.cashTotal,
      creditSales: stats.creditTotal,
    });

    const audit = await findByToken(closeToken);
    return {
      till: closedTill,
      closeToken,
      audit,
      expectedCloseFloat: cashTotals.expectedClose,
      cashSalesTotal: stats.cashTotal,
      creditSalesTotal: stats.creditTotal,
    };
  }
