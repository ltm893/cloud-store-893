...
#
# Usage (from android-lister/):
#   ./RebuildReinstall.sh
#
# Optional env:
#   API_BASE_URL=https://dev.oci.cloudstore893.com/   Full cloud URL (overrides OCI_API_HOST)
#   RELEASE_API_BASE_URL=https://oci.cloudstore893.com/ Same as API_BASE_URL (honored as-is)
...
#   USE_LOCAL=1             Auto-detect Mac LAN IP (not OCI)
...
if [[ -n "${API_BASE_URL:-}" ]]; then
  ...
  unset LAN_IP
  echo "==> API_BASE_URL=${RELEASE_API_BASE_URL} (from API_BASE_URL)"
elif [[ -n "${RELEASE_API_BASE_URL:-}" ]]; then
  export RELEASE_API_BASE_URL="${RELEASE_API_BASE_URL%/}/"
  unset LAN_IP
  echo "==> API_BASE_URL=${RELEASE_API_BASE_URL} (from RELEASE_API_BASE_URL)"
else
  if [[ -n "${LAN_IP:-}" ]]; then
    API_HOST="$LAN_IP"
...
echo "==> Done. Cloud Store Lister debug APK installed (API_BASE_URL=${RELEASE_API_BASE_URL})."
